<template>
  <NuxtLink :to="`/thoughts/${blog.slug}`" class="cursor-pointer">
    <div
      ...
      v-if="blog.metadata"
    >
      <header class="flex items-center justify-between hover:px-4 focused:px-4 pt-4 pb-2">
        <h1
      ...
      <p class="font-mono text-gray-500 dark:text-gray-400 hover:px-4 focused:px-4 pt-0 font-medium">
        Updated {{ blog.modified_at | moment("from", "now") }}
      </p>
      <div class="flex space-x-2 hover:pl-4 focused:pl-4 pb-4">
        <span
      ...
      <p
        class="text-gray-700 dark:text-gray-300 hover:px-4 focused:px-4 pt-2 pb-4 font-medium mb-0"
      >
